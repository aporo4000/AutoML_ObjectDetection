...
DATA=`cat $1/detect_train_output.txt`
...
do
    echo $line
    curl -X GET -H "$HEAD" -H "$HEAD1" $URL$line > $1/detect_progress_train_$PROJECT_ID.json
...
python output_progress_train_replace.py -u $1
