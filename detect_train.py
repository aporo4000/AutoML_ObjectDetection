...
def create_model(project_id, compute_region, user_folder, model_name, dataset_id):
  ...
  # デフォルト設定以下(予測精度が悪い場合ここを調整：課金額が上がる)
  ##my_model = {
  ##    'display_name': model_name,
  ##    'dataset_id': dataset_id,
  ##    'image_object_detection_model_metadata': {
  ##        'nodeCount': '9',  << 記載不要で9ノード作られる
...
##   }
  ...
  ###
  with open(user_folder+'/detect_train_output.txt', 'w') as f:
...
if __name__ == '__main__':
  ...
  create_model_parser.add_argument('-u')
  ...
  if args.command == 'create_model':
    ...
    create_model(project_id, compute_region, args.u, args.model_name, args.dataset_id)
